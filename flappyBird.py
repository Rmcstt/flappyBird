...
def reiniciar_jogo():
    global pontos, passaros, canos, chao
    pontos = 0
    passaros = [Passaro(230, 350)]
    chao = Chao(730)
    canos = [Cano(700)]

def desenhar_tela(tela, passaros, canos, chao, pontos, botao_reload=None):
    ...
    while rodando:
        ...
        for evento in pygame.event.get():
            if evento.type == pygame.QUIT:
                rodando = False
                pygame.quit()
                botao_reload = Button("Reload", 200, 400, 100, 50, (0, 255, 0), (0, 200, 0), reload_game)
                quit()
            if evento.type == pygame.KEYDOWN:
                if evento.key == pygame.K_SPACE:
                    for passaro in passaros:
                        passaro.pular()
            if botao_reload:
                botao_reload.handle_event(evento)
        ...
        desenhar_tela(tela, passaros, canos, chao, pontos, botao_reload)
